...
export default function Header(props) {
  return (
    <header id="headerId" className={props.homePage ? "flex w-full justify-between items-center px-5 lg:px-10 py-5 top-0 z-[101] fixed font-medium drop-shadow bg-transparent text-white transition-five" : "backdrop-filter backdrop-blur-sm flex w-full justify-between items-center px-5 lg:px-10 py-5 top-0 z-[101] fixed font-medium text-gray-900 transition-five"} style={{ fontWeight: "600", fontSize: "1.1rem" }}>
      <div class="flex items-center gap-4">
        <div>
          <span className="headerSpan" style={{ width: "3.2rem", top: "1.2px" }}>
        ...
        <div class="text-xl font-semibold cursor-pointer hover:border-b-2 hover:border-b-primary transition-one border-b-2 border-b-primary">
          <div>
            <span className='headerSpan' style={{ width: "10rem" }}>
              <img alt="logo" src={props.productName} class="cursor-pointer headerImg" />
            </span>
          </div>
        </div>
      </div>

      <nav class="hidden lg:flex lg:flex-[.8] xl:flex-[.6]">
